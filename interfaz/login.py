...
from interfaz_estudiante.principal_estudiantes import inicial_estudiantes
...
class Login(ctk.CTkToplevel):
    ...
    def siguiente_ventana(self, pase):
        self.withdraw()
        if pase == "Estudiante":
            if self.inside_estudiante is None or not self.inside_estudiante.winfo_exists():
                self.inside_estudiante = inicial_estudiantes(self)
                # La cédula se obtiene automáticamente desde el entry_usuario
            else:
                self.inside_estudiante.deiconify()
                
        elif pase == "Docente":
            if self.inside_docente is None or not self.inside_docente.winfo_exists():
                ...
                self.inside_docente.deiconify()
                
        elif pase == "Administrador":
            if self.inside_personal is None or not self.inside_personal.winfo_exists():
                ...
                self.inside_personal.deiconify()